...
def mything():
    ...
    yout, xout, weights = loess_1d(x,y, frac=0.2, degree=1)
    plt.plot(yout, xout, label='LOESS_f=2_d=1')
    yout, xout, weights = loess_1d(x,y, frac=0.8, degree=1)
    plt.plot(yout, xout, label='LOESS_f=5_d=1')
    yout, xout, weights = loess_1d(x,y, frac=0.2, degree=2)
    plt.plot(yout, xout, label='LOESS_f=2_d=2')
    yout, xout, weights = loess_1d(x,y, frac=0.8, degree=2)
    plt.plot(yout, xout, label='LOESS_f=8_d=2')
    plt.legend()
    ##################
    ################# 
